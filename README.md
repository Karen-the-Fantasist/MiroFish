...
| **Docker** | 最新版 | 运行 Neo4j 容器 | `docker --version` |
...
| 服务 | 端口 | 说明 |
|------|------|------|
| Neo4j HTTP | http://127.0.0.1:7474 | 图数据库管理界面 |
| Neo4j Bolt | bolt://127.0.0.1:7687 | 图数据库连接（用户名: neo4j，密码: mirofish123）|
| 后端 API | http://127.0.0.1:5001 | Flask API 服务 |
...
- **本地开发**：核心代码修改在本地进行，使用 `npm run dev` 启动服务
- **Neo4j 依赖**：Neo4j 通过 Docker Compose 管理，`npm run dev` 会自动启动
- **禁止构建 MiroFish Docker 镜像**：开发时不需要构建镜像，所有开发均在本地环境进行
