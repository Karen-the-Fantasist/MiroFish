...
def _read_text_with_fallback(file_path: str) -> str:
    ...
    try:
        ...
        logger.debug(
            f"[FALLBACK] _read_text_with_fallback UTF-8解码失败，尝试其他编码 | "
            f"file_path={file_path}, data_len={len(data)} | "
    ...
    try:
        ...
        logger.debug(
            f"[FALLBACK] _read_text_with_fallback charset_normalizer失败，尝试chardet | "
            f"file_path={file_path}, data_len={len(data)} | "
            f"exception={type(e).__name__}: {str(e)[:100]}"
        )
    ...
    if not encoding:
        try:
            ...
            logger.debug(
                f"[FALLBACK] _read_text_with_fallback chardet失败，使用UTF-8+replace | "
                f"file_path={file_path}, data_len={len(data)} | "
                f"exception={type(e).__name__}: {str(e)[:100]}"
            )
    ...
    def extract_from_multiple(cls, file_paths: List[str]) -> str:
        ...
        for i, file_path in enumerate(file_paths, 1):
            try:
                ...
                logger.warning(
                    f"[FALLBACK] extract_from_multiple 跳过失败文件 | "
                    f"index={i}, file_path={file_path}, total_files={len(file_paths)} | "
