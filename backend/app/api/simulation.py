...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        if not entity:
            return jsonify(
                {"success": False, "error": f"实体不存在: {entity_uuid}"}
            ), 404
...
def create_simulation():
    ...
    try:
        ...
        if not project:
            return jsonify(
                {"success": False, "error": f"项目不存在: {project_id}"}
            ), 404
...
def prepare_simulation():
    ...
    try:
        ...
        if not force_regenerate:
            ...
            logger.debug(
                f"检查结果: is_prepared={is_prepared}, prepare_info={prepare_info}"
            )
            if is_prepared:
        ...
        if not project:
            return jsonify(
                {"success": False, "error": f"项目不存在: {state.project_id}"}
            ), 404
...
def get_prepare_status():
    ...
    try:
        ...
        if not task_id:
            ...
            return jsonify(
                {"success": False, "error": "请提供 task_id 或 simulation_id"}
            ), 400
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify(
                {"success": False, "error": f"模拟配置不存在，请先调用 /prepare 接口"}
            ), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify(
                {"success": False, "error": "配置文件不存在，请先调用 /prepare 接口"}
            ), 404
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        if not os.path.exists(script_path):
            return jsonify(
                {"success": False, "error": f"脚本文件不存在: {script_name}"}
            ), 404
...
def generate_profiles():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({"success": False, "error": "请提供 graph_id"}), 400
        ...
        if filtered.filtered_count == 0:
            return jsonify({"success": False, "error": "没有找到符合条件的实体"}), 400
    ...
    try:
        ...
        if max_rounds is not None:
            try:
                ...
                if max_rounds <= 0:
                    return jsonify(
                        {"success": False, "error": "max_rounds 必须是正整数"}
                    ), 400
            except (ValueError, TypeError):
                return jsonify(
                    {"success": False, "error": "max_rounds 必须是有效的整数"}
                ), 400
            ...
            if is_prepared:
                ...
                if force:
                    ...
                    if not cleanup_result.get("success"):
                        logger.warning(
                            f"清理日志时出现警告: {cleanup_result.get('errors')}"
                        )
        ...
        if enable_graph_memory_update:
            ...
            logger.info(
                f"启用图谱记忆更新: simulation_id={simulation_id}, graph_id={graph_id}"
            )
...
def interview_agent():
    ...
    try:
        ...
        if not prompt:
            return jsonify(
                {"success": False, "error": "请提供 prompt（采访问题）"}
            ), 400

        # 验证platform参数
        if platform and platform not in ("twitter", "reddit"):
            return jsonify(
                {"success": False, "error": "platform 参数只能是 'twitter' 或 'reddit'"}
            ), 400
        ...
        return jsonify(
            {"success": False, "error": f"等待Interview响应超时: {str(e)}"}
        ), 504
...
def interview_agents_batch():
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify(
                {"success": False, "error": "请提供 interviews（采访列表）"}
            ), 400
        ...
        for i, interview in enumerate(interviews):
            if "agent_id" not in interview:
                return jsonify(
                    {"success": False, "error": f"采访列表第{i + 1}项缺少 agent_id"}
                ), 400
            if "prompt" not in interview:
                return jsonify(
                    {"success": False, "error": f"采访列表第{i + 1}项缺少 prompt"}
                ), 400
            # 验证每项的platform（如果有）
        ...
        return jsonify(
            {"success": False, "error": f"等待批量Interview响应超时: {str(e)}"}
        ), 504
...
def interview_all_agents():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"等待全局Interview响应超时: {str(e)}"}
        ), 504
