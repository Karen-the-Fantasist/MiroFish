...
from ..adapters.zep_graph_adapter import ZepGraphAdapter
...
class ZepToolsService:
    ...
    def __init__(
        ...
        # api_key 保留参数以保持接口兼容性，ZepGraphAdapter 不需要它
        self.api_key = api_key

        self.client = ZepGraphAdapter()
        # LLM客户端用于InsightForge生成子问题
        self._llm_client = llm_client
        logger.info("ZepToolsService 初始化完成 (使用 ZepGraphAdapter)")
    ...
    def search_graph(
        ...
        # 尝试使用ZepGraphAdapter Search API
        try:
            ...
            # 解析事实（ZepGraphAdapter返回facts为dict列表）
            if hasattr(search_results, "facts") and search_results.facts:
                for fact_item in search_results.facts:
            ...
            if hasattr(search_results, "edges") and search_results.edges:
                for edge in search_results.edges:
                    if hasattr(edge, "fact") and edge.fact:
                        if edge.fact not in facts:
                            facts.append(edge.fact)
                    edges.append(
                        {
                            "uuid": getattr(edge, "uuid", None)
                            or getattr(edge, "uuid_", ""),
            ...
            if hasattr(search_results, "nodes") and search_results.nodes:
                for node in search_results.nodes:
                    ...
                    if hasattr(node, "summary") and node.summary:
                        node_fact = f"[{node.name}]: {node.summary}"
                        if node_fact not in facts:
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        while True:
            batch = self.client.node.get_by_graph_id(
                graph_id=graph_id, limit=page_size, uuid_cursor=cursor
            )
            if not batch:
                break
            ...
            if len(all_nodes) >= max_nodes:
                ...
                logger.warning(f"节点数量达到限制 ({max_nodes})，停止分页")
                break
            if len(batch) < page_size:
                break

            cursor = getattr(batch[-1], "uuid", None) or getattr(
                batch[-1], "uuid_", None
            )
            if not cursor:
                break
        ...
        for node in all_nodes:
            node_uuid = (
                getattr(node, "uuid", None) or getattr(node, "uuid_", None) or ""
            )
            result.append(
                NodeInfo(
    ...
    def get_all_edges(
        ...
        while True:
            batch = self.client.edge.get_by_graph_id(
                graph_id=graph_id, limit=page_size, uuid_cursor=cursor
            )
            if not batch:
                break
        ...
        for edge in all_edges:
            edge_uuid = (
                getattr(edge, "uuid", None) or getattr(edge, "uuid_", None) or ""
            )
            edge_info = EdgeInfo(
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            node = self._call_with_retry(
                func=lambda: self.client.node.get(uuid_=node_uuid),
            ...
            return NodeInfo(
                uuid=getattr(node, "uuid", None) or getattr(node, "uuid_", ""),
                name=node.name or "",
                labels=list(node.labels) if node.labels else [],
                summary=node.summary or "",
                attributes=dict(node.attributes) if node.attributes else {},
    ...
    def interview_agents(
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # 始终输出双平台标记
                twitter_text = (
                    twitter_response if twitter_response else "（该平台未获得回复）"
                )
                reddit_text = (
                    reddit_response if reddit_response else "（该平台未获得回复）"
                )
