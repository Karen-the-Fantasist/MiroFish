...
class SimulationRunner:
    ...
    def _load_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        try:
            ...
            logger.error(
                f"[FALLBACK] _load_run_state 返回 None | "
                f"simulation_id={state.simulation_id if state else 'unknown'} | "
    ...
    def start_simulation(
        ...
        try:
            ...
            logger.info(
                f"模拟启动成功: {simulation_id}, pid={process.pid}, platform={platform}"
            )
        ...
        try:
            ...
            if exit_code == 0:
                ...
                try:
                    ...
                    logger.debug(
                        f"[FALLBACK] _monitor_simulation 跳过读取错误日志 | "
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    if all_completed:
                                        ...
                                        state.completed_at = datetime.now().isoformat()
                                        logger.info(
                                            f"所有平台模拟已完成: {state.simulation_id}"
                                        )
                            ...
                            logger.debug(
                                f"[FALLBACK] _read_action_log 跳过无效JSON行 | "
                                f"log_path={log_path}, line_num=~{position} | "
                                f"exception={type(e).__name__}: {str(e)[:100]}"
                            )
                return f.tell()
        except Exception as e:
            logger.warning(
                f"[FALLBACK] _read_action_log 返回当前位置 | "
                f"log_path={log_path}, position={position}, platform={platform} | "
                f"exception={type(e).__name__}: {str(e)[:200]}"
            )
            return position
    ...
    def _terminate_process(
        ...
        if IS_WINDOWS:
            ...
            logger.info(
                f"终止进程树 (Windows): simulation={simulation_id}, pid={process.pid}"
            )
            try:
                # 先尝试优雅终止
                subprocess.run(
                ...
                logger.warning(
                    f"[FALLBACK] _terminate_process Windows taskkill失败，使用terminate | "
                    f"simulation_id={simulation_id}, pid={process.pid} | "
                    f"exception={type(e).__name__}: {str(e)[:200]}"
                )
                process.terminate()
                try:
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if process and process.poll() is None:
            try:
                ...
                # 进程已经不存在
                logger.debug(
                    f"[FALLBACK] stop_simulation 进程已不存在 | "
                    f"simulation_id={simulation_id}, pid={process.pid}"
                )
            except Exception as e:
                logger.error(
                    f"[FALLBACK] stop_simulation 进程组终止失败，回退到terminate | "
                    f"simulation_id={simulation_id}, pid={process.pid} | "
                    f"exception={type(e).__name__}: {str(e)[:200]}"
                )
                # 回退到直接终止进程
                try:
                    ...
                    logger.warning(
                        f"[FALLBACK] stop_simulation terminate失败，使用kill | "
                        f"simulation_id={simulation_id}, pid={process.pid} | "
                        f"exception={type(e2).__name__}: {str(e2)[:100]}"
                    )
                    process.kill()
        ...
        if cls._graph_memory_enabled.get(simulation_id, False):
            try:
                ...
                logger.error(
                    f"[FALLBACK] stop_simulation 停止图谱记忆更新器失败 | "
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    logger.debug(
                        f"[FALLBACK] _read_actions_from_file 跳过无效JSON行 | "
                        f"file_path={file_path}, line_preview={line[:50] if line else None} | "
                        f"exception={type(e).__name__}: {str(e)[:100]}"
                    )
                    continue
        ...
        for filename in files_to_delete:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    logger.debug(
                        f"[FALLBACK] cleanup_simulation_logs 删除文件失败 | "
                        f"simulation_id={simulation_id}, filename={filename} | "
                        f"exception={type(e).__name__}: {str(e)[:100]}"
                    )
            ...
            if os.path.exists(dir_path):
                ...
                if os.path.exists(actions_file):
                    try:
                        ...
                        logger.debug(
                            f"[FALLBACK] cleanup_simulation_logs 删除动作日志失败 | "
                            f"simulation_id={simulation_id}, dir_name={dir_name} | "
                            f"exception={type(e).__name__}: {str(e)[:100]}"
                        )
    ...
    def cleanup_all_simulations(cls):
        ...
        try:
            ...
            logger.error(
                f"[FALLBACK] cleanup_all_simulations 停止图谱记忆更新器失败 | "
                f"exception={type(e).__name__}: {str(e)[:200]}"
            )
            ...
            try:
                if process.poll() is None:  # 进程仍在运行
                    ...
                    try:
                        ...
                        # 进程可能已经不存在，尝试直接终止
                        logger.debug(
                            f"[FALLBACK] cleanup_all_simulations 进程不存在，使用terminate | "
                            f"simulation_id={simulation_id}, pid={process.pid} | "
                            f"exception={type(e).__name__}: {str(e)[:100]}"
                        )
                        try:
                            ...
                            logger.debug(
                                f"[FALLBACK] cleanup_all_simulations terminate失败，使用kill | "
                                f"simulation_id={simulation_id}, pid={process.pid} | "
                                f"exception={type(e2).__name__}: {str(e2)[:100]}"
                            )
                            process.kill()
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[FALLBACK] cleanup_all_simulations 更新state.json失败 | "
                            f"simulation_id={simulation_id}, state_file={state_file} | "
                            f"exception={type(state_err).__name__}: {str(state_err)[:200]}"
                        )

            except Exception as e:
                logger.error(
                    f"[FALLBACK] cleanup_all_simulations 清理进程失败 | "
                    f"simulation_id={simulation_id} | "
                    f"exception={type(e).__name__}: {str(e)[:200]}"
                )
        ...
        for simulation_id, file_handle in list(cls._stdout_files.items()):
            try:
                ...
                logger.debug(
                    f"[FALLBACK] cleanup_all_simulations 关闭stdout文件句柄失败 | "
                    f"simulation_id={simulation_id} | "
                    f"exception={type(e).__name__}: {str(e)[:100]}"
                )
        ...
        for simulation_id, file_handle in list(cls._stderr_files.items()):
            try:
                ...
                logger.debug(
                    f"[FALLBACK] cleanup_all_simulations 关闭stderr文件句柄失败 | "
                    f"simulation_id={simulation_id} | "
                    f"exception={type(e).__name__}: {str(e)[:100]}"
                )
                pass
