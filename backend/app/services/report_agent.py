...
class ReportLogger:
    ...
    def log_planning_context(self, context: Dict[str, Any]):
        ...
        self.log(
            ...
            stage="planning",
            details={"message": "获取模拟上下文信息", "context": context},
        )
    ...
    def log_planning_complete(self, outline_dict: Dict[str, Any]):
        ...
        self.log(
            ...
            stage="planning",
            details={"message": "大纲规划完成", "outline": outline_dict},
        )
...
class ReportAgent:
    ...
    def _execute_tool(
        ...
        try:
            ...
            logger.warning(
                f"[FALLBACK] _execute_tool fallback behavior | "
                f"tool_name={tool_name}, parameters={parameters} | "
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(xml_pattern, response, re.DOTALL):
            try:
                ...
                logger.debug(
                    f"[FALLBACK] _parse_tool_calls JSON decode skipped (format 1) | "
                    f"json_snippet={match.group(1)[:100]}... | "
        ...
        if stripped.startswith("{") and stripped.endswith("}"):
            try:
                ...
                logger.debug(
                    f"[FALLBACK] _parse_tool_calls JSON decode skipped (format 2) | "
        ...
        if match:
            try:
                ...
                logger.debug(
                    f"[FALLBACK] _parse_tool_calls JSON decode skipped (format 3) | "
                    f"json_snippet={match.group(1)[:100]}... | "
                    f"exception={type(e).__name__}: {str(e)[:100]}"
                )
    ...
    def plan_outline(
        ...
        try:
            ...
            logger.warning(
                f"[FALLBACK] plan_outline fallback behavior | "
    ...
    def _generate_section_react(
        ...
        response = self.llm.chat(messages=messages, temperature=0.5, max_tokens=40960)
    ...
    def generate_report(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"[FALLBACK] generate_report save failure fallback (data loss) | "
                    f"report_id={report_id} | "
                    f"exception={type(save_error).__name__}: {str(save_error)[:200]}"
                )
    ...
    def chat(
        ...
        try:
            ...
            logger.warning(
                f"[FALLBACK] chat fallback behavior (empty report_content) | "
                f"simulation_id={self.simulation_id} | "
                f"exception={type(e).__name__}: {str(e)[:200]}"
            )
