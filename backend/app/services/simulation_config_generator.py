        ...
        for attempt in range(max_attempts):
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"[FALLBACK] _call_llm_with_retry JSON repair fallback | "
                        f"attempt={attempt + 1}/{max_attempts}, model={self.model_name} | "
                ...
                logger.warning(
                    f"[FALLBACK] _call_llm_with_retry backoff retry fallback | "
                    f"attempt={attempt + 1}/{max_attempts}, model={self.model_name} | "
                    f"exception={type(e).__name__}: {str(e)[:200]}"
                )
                last_error = e
    ...
    def _try_fix_config_json(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        if json_match:
            ...
            try:
                ...
                logger.debug(
                    f"[FALLBACK] _try_fix_config_json alternative fix fallback | "
                    f"content_len={len(json_str)} | "
                    f"exception={type(e).__name__}: {str(e)[:200]}"
                )
                # 尝试移除所有控制字符
                ...
                try:
                    ...
                    logger.debug(
                        f"[FALLBACK] _try_fix_config_json control char fix fallback failed | "
                        f"content_len={len(json_str)} | "
                        f"exception={type(e2).__name__}: {str(e2)[:200]}"
                    )
...
- reasoning (string): 简要说明为什么这样配置"""
        ...
        try:
            ...
            logger.warning(
                f"[FALLBACK] _generate_time_config default fallback | "
...
{{
    ...
    "reasoning": "<简要说明>"
        ...
        try:
            ...
            logger.warning(
                f"[FALLBACK] _generate_event_config empty default fallback | "
...
{{
    "agent_configs": [
        ...
        try:
            ...
            logger.warning(
                f"[FALLBACK] _generate_agent_configs_batch rule-based fallback | "
                f"batch_size={len(entities)}, start_idx={start_idx} | "
                f"exception={type(e).__name__}: {str(e)[:200]}"
            )
            llm_configs = {}
