...
class SimulationIPCClient:
    ...
    def send_command(
            ...
            if os.path.exists(response_file):
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"[FALLBACK] send_command 清理IPC文件失败 | "
                            f"command_id={command_id}, command_file={command_file} | "
        ...
        try:
            ...
            logger.debug(
                f"[FALLBACK] send_command 清理超时命令文件失败 | "
                f"command_id={command_id}, command_file={command_file} | "
                f"exception={type(e).__name__}: {str(e)[:100]}"
            )
    ...
    def check_env_alive(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"[FALLBACK] check_env_alive 返回False | "
                f"simulation_dir={self.simulation_dir}, status_file={status_file} | "
                f"exception={type(e).__name__}: {str(e)[:100]}"
            )
            return False
...
class SimulationIPCServer:
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        ...
        for filepath, _ in command_files:
            try:
                ...
                logger.warning(
                    f"[FALLBACK] poll_commands 跳过无效命令文件 | "
                    f"filepath={filepath} | "
    ...
    def send_response(self, response: IPCResponse):
        ...
        try:
            ...
            logger.debug(
                f"[FALLBACK] send_response 清理命令文件失败 | "
                f"command_id={response.command_id}, command_file={command_file} | "
                f"exception={type(e).__name__}: {str(e)[:100]}"
            )
